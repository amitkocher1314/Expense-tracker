...
const ExpensePage = () => {
  ...
  const handleDownloadCSV = () => {
    const csvRows = [
      ...
      ...expenses.map(exp => [exp.id, exp.amount, exp.description, exp.category, exp.date])
    ];
  ...
  return (
    <div className={`flex flex-col min-h-screen ${theme}`}>
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-4">
          <label htmlFor="filter-category" className="block text-sm font-medium text-gray-700">Filter by Category</label>
          <select
            id="filter-category"
        ...
        {isModalOpen && (
          <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-70" onClick={closeModal}>
            <div className="bg-white p-8 rounded-md shadow-md max-w-md w-full" onClick={(e) => e.stopPropagation()}>
